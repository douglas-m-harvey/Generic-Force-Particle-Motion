...
#                print("force on " + str(i) + " from " + str(j) + " = " + str(f_x))
    ...
    for i in range(0, int(float(np.shape(P)[0]))):
        p_x = float(P[i].position_x[k - 1] + P[i].velocity_x[k - 1]*t_step)
        p_y = float(P[i].position_y[k - 1] + P[i].velocity_y[k - 1]*t_step)
        p_z = float(P[i].position_z[k - 1] + P[i].velocity_z[k - 1]*t_step)
